        ...
        <div className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
              className={`relative px-2 py-1 text-sm uppercase tracking-wider transition-colors ${
                isActive(item.path) 
            ...
            >
              {item.name}
              {isActive(item.path) && (
                <span className="absolute bottom-0 left-0 w-full h-0.5 bg-navy-glow" />
              )}
            </NavLink>
          ))}
        </div>
          ...
          <div className="flex flex-col space-y-4">
            {navItems.map((item) => (
              <NavLink
                key={item.name}
                to={item.path}
                className={`px-2 py-3 text-sm uppercase tracking-wider border-b border-white/5 ${
                  isActive(item.path) 
                    ? 'text-white font-medium' 
                    : 'text-gray-400 hover:text-white'
                }`}
